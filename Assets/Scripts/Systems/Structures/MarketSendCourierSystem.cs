...
{
    public class MarketSendCourierSystem : IEcsInitSystem, IEcsRunSystem
    {
        ...
        public void Run(IEcsSystems systems)
        {
            ...
            var workFilter = world.Filter<MarketSendCourier>().End();
            if (workFilter.GetEntitiesCount() == 0) return;

            var marketFilter = world.Filter<Market>().Inc<RoadAccess>().Inc<HasWorkforce>().Inc<MarketSendCourier>().Exc<WorkInProgress>().End();
            ...
            foreach (var hunterEntity in hunterFilter)
            {
                ...
                foreach (var marketEntity in marketFilter)
                {
                    ...
                    var path = _gridPathfinder.GetAStarSearchPath(_grid, startPosition, endPosition, PathType.Road, false);
                    ...
                    {
                        Prefab = _prefabSetup.MarketCourierCharacterPrefab,
